...
function InsightWatcher(args) { 
	...
	this.init = function(args) {
		self.insight_servers = args.insight_servers;
		self.insight_apis_servers = args.insight_api_endpoints;
		self.crypto_symbol = args.crypto_symbol;
	}
	this.getAddress = function(address) {
		...
		return new Promise(function(Success, Reject) {
			...
			result.in = 0
			result.out = 0
			result.curr = `bits(u${self.crypto_symbol})`
			console.log(self.api_url + 'txs/?address=' + address);
				...
				if (self.url != undefined) {
					...
					self.socket.on('tx', function(data) {
						...
						data.vout.forEach(function(each_vout) {
							...
							self.events.emit(Object.keys(each_vout)[0], { data: data, address: Object.keys(each_vout)[0], amount: each_vout[Object.keys(each_vout)[0]] });
						})
